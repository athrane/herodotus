...
describe('RealmStateComponent', () => {
  ...
  describe('Factory Methods', () => {
    describe('create()', () => {
      ...
      it('should create instance with custom faction data', () => {
        const factionInfluence = new Map([['Military', 50], ['Clergy', 30]]);
        const factionAllegiance = new Map([['Military', 60], ['Clergy', 70]]);
      ...
      it('should validate factionInfluence parameter type', () => {
        expect(() => {
          RealmStateComponent.create(100, 50, 50, 0, 'invalid');
        }).toThrow(TypeError);
      });

      it('should validate factionAllegiance parameter type', () => {
        expect(() => {
          RealmStateComponent.create(100, 50, 50, 0, new Map(), 'invalid');
        }).toThrow(TypeError);
      });
    ...
    describe('createNull()', () => {
      ...
      it('should have empty faction maps', () => {
        const nullComponent = RealmStateComponent.createNull();
  ...
  describe('Faction Management', () => {
    describe('Faction Influence', () => {
      ...
      it('should check if faction influence exists', () => {
        const component = RealmStateComponent.create();
        component.setFactionInfluence('Merchants', 50);
        expect(component.hasFactionInfluence('Merchants')).toBe(true);
        expect(component.hasFactionInfluence('NonExistent')).toBe(false);
      });
      ...
      it('should validate faction name in hasFactionInfluence', () => {
        const component = RealmStateComponent.create();
        expect(() => {
          component.hasFactionInfluence('');
        }).toThrow(TypeError);
      });
    });
    ...
    describe('Faction Allegiance', () => {
      ...
      it('should check if faction allegiance exists', () => {
        const component = RealmStateComponent.create();
        component.setFactionAllegiance('Merchants', 65);
        expect(component.hasFactionAllegiance('Merchants')).toBe(true);
        expect(component.hasFactionAllegiance('NonExistent')).toBe(false);
      });
      ...
      it('should validate faction name in hasFactionAllegiance', () => {
        const component = RealmStateComponent.create();
        expect(() => {
          component.hasFactionAllegiance('');
        }).toThrow(TypeError);
      });
    });
  });
